    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            res.headers_mut().insert(header::CONTENT_SECURITY_POLICY, header::HeaderValue::from_str(&csp).unwrap());
            res.headers_mut().insert(header::X_CONTENT_TYPE_OPTIONS, header::HeaderValue::from_static("nosniff"));
            res.headers_mut().insert(header::X_FRAME_OPTIONS, header::HeaderValue::from_static("DENY"));
            res.headers_mut().insert(header::X_XSS_PROTECTION, header::HeaderValue::from_static("1; mode=block"));
